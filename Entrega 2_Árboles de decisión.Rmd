...
```{r}
# Definir los límites con cuartiles
q1 <- quantile(train_set$SalePrice, 0.33, na.rm = TRUE)  # 33% más barato
q2 <- quantile(train_set$SalePrice, 0.66, na.rm = TRUE)  # 66% intermedio

# Crear la categoría basada en SalePrice
train_set$Categoria <- cut(train_set$SalePrice, 
...
# Convertir la variable `Categoria` a factor (categórica)
train_set$Categoria <- as.factor(train_set$Categoria)

# Excluir SalePrice (porque ya se usó para generar Categoria)
train_set <- train_set %>% select(-SalePrice)

# Crear el modelo de árbol de clasificación
set.seed(42)
arbol_clasificacion <- rpart(Categoria ~ ., data = train_set, method = "class")

# Visualizar el árbol de clasificación
rpart.plot(arbol_clasificacion, type = 3, extra = 104, fallen.leaves = TRUE, cex = 0.6, 
